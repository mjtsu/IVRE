...
def add_object(object_dir, name, rgba, scale, loc, theta=0):
    """
    ...
    shape, mat, color = name
    new_name = "obj_%s_%s_%s" % (shape, mat, color)
    x, y = loc
    if new_name not in bpy.data.objects:
        filename = os.path.join(object_dir, "%s.blend" % shape, "Object", shape)
        bpy.ops.wm.append(filename=filename)
        bpy.data.objects[shape].name = new_name
        bpy.context.view_layer.objects.active = bpy.data.objects[new_name]
        add_material(mat, Color=rgba)

    new_object = bpy.data.objects[new_name]
    new_object.hide_render = False
    new_object.hide_viewport = False
    new_object = bpy.data.objects[new_name]
    new_object.rotation_euler = (0, 0, theta)
    new_object.location = (x, y, scale)
    new_object.scale = (scale, scale, scale)
...
def camera_view_bounds_2d(context, cam_ob, me_ob):
    ...
    me = me_ob.evaluated_get(depsgraph).to_mesh()

    me.transform(me_ob.matrix_world)
    me.transform(mat)

    camera = cam_ob.data
    frame = [-v for v in camera.view_frame(scene=context.scene)[:3]]
...
def removeDataBlocks():
    """
    adopted from https://github.com/CNCLgithub/object-cleanser/blob/master/blenderClass.py
    """
    # Removes unlinked data blocks and prevents memory leakage
    toRemove = [block for block in bpy.data.meshes if block.users == 0]
    ...
    toRemove = [block for block in bpy.data.textures if block.users == 0]
    for block in toRemove:
        bpy.data.textures.remove(block, do_unlink=True)

    toRemove = [block for block in bpy.data.images if block.users == 0]
    for block in toRemove:
