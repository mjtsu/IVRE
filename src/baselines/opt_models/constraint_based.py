...
class PCModel(BaseModel):
    ...
    def train(self, context):
        ...
        def robust_test(y, x, z, data, alpha):
            ...
            """
            regression = sm.RLM(
                np.take(data, [y], axis=1), np.take(data, [x] + list(z), axis=1)
            )
            result = regression.fit()
            coefficient = result.params[0]
            confidence_interval = result.conf_int(alpha=alpha / 2.0)
    ...
    def build_cpt(self, context):
        ...
        for cpt_idx in self.cpt:
            self.cpt[cpt_idx]["blicketness"] = self.cpt[cpt_idx]["on"] / (
                self.cpt[cpt_idx]["on"] + self.cpt[cpt_idx]["off"]
            )
