...
class NoTearsLinear(BaseModel):
    ...
    def fit(
        ...
        def _func(w):
            ...
            # print(loss, rho, h, alpha, lambda1, w.sum())
            # if obj > 2e6:
            #     print(np.sum(self.context,axis=1))
        ...
        def _func(var):
            if loss_type == "logistics":
                ...
                G_loss = (
                    ...
                    / n
                    * (
                        (1.0 - 1.0 / (1.0 + np.exp(M)) - X) @ self.W_est[-1, :]
                        - M[:, -1]
                    )
                )
            else:
...
class NoTearsMLP(NoTearsLinear):
    ...
    def fit(
        ...
        def dual_ascent_step(
            loss_func, model, X, lambda1, lambda2, rho, alpha, h, rho_max
        ):
        ...
        def log_loss(output, target):
            ...
            loss = (
                ...
                / n
                * torch.sum(
                    target * torch.log(sigmoid)
                    + (1.0 - target) * torch.log(1.0 - sigmoid)
                )
            )
            return loss
    ...
    def solve(self, query, loss_type):
        ...
        def _func(var):
            if loss_type == "logistics":
                ...
                var_torch = torch.from_numpy(var[:, np.newaxis]).to(
                ...
                loss = (
                    ...
                    / n
                    * torch.sum(
                        X * torch.log(sigmoid) + (1.0 - X) * torch.log(1.0 - sigmoid)
                    )
                )
