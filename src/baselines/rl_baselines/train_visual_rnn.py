...
import gym
from IVRECore import IVRE, blicketWrapper
import numpy as np
import torch
...
def test_ddpg(args=get_args()):
    ...
    if args.training_num > 1:
        train_envs = SubprocVectorEnv(
            [lambda: blicketWrapper(IVRE()) for _ in range(args.training_num)]
        )
    else:
        train_envs = blicketWrapper(IVRE())

    test_envs = blicketWrapper(IVRE())
    ...
    if args.training_num > 1:
        buffer = VectorReplayBuffer(
            args.buffer_size, len(train_envs), stack_num=args.stack_num
        )
    else:
