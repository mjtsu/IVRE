...
from networks import ProcessLSTM
from tianshou.utils.net.continuous import Actor, Critic


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--task", type=str, default="Interactive_Blicket_DDPG_RNN")
...
def train_ddpg_rnn(args=get_args()):
    ...
    log_file = f'seed_{args.seed}_{t0}-{args.task.replace("-", "_")}_ddpg_rnn'
    log_path = os.path.join(args.logdir, args.task, "ddpg_rnn", log_file)
...
def test_ddpg_rnn(args=get_args()):
    env = SymbolicIVRE(IVRE())
    args.state_shape = env.observation_space.shape or env.observation_space.n
    args.action_shape = env.action_space.shape or env.action_space.n
    args.max_action = env.action_space.high[0]
