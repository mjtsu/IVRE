...
class IVRE(gym.Env):
    ...
    def __init__(self, config_size=ALL_CONFIG_SIZE):
        ...
        self.action_space = spaces.Dict({
            "trial":
            spaces.MultiBinary(config_size),  # Agent's trial
            "belief":
            spaces.Box(
                low=0.0,
                high=1.0,
        ...
        self.observation_space = spaces.Dict({
            "available_objects":
            spaces.MultiBinary(
                config_size),  # Available objects in current episode
            "context":
            spaces.MultiBinary(
                config_size),  # Last Trial/ Initial panel objects
            "light_state":
            spaces.MultiBinary(
                1),  # Last Trial/ Initial panel Blicket Machine State
            "last_belief":
            spaces.Box(
                low=0.0,
                high=1.0,
    ...
    def reward(self, belief):
        return self._reward(self, belief)
    ...
    def _reward(self, belief):
        ...
        for index, prob in enumerate(belief_convert):
            if (self.available_objects[index] and
                    abs(self.true_label[index] - prob) >= CONFIDENCE_THRES):
                return -self._jsd(
                    oracle, belief_convert
                ) - 1.0  # JSD of Agent's Blief and Oracle Solver's result

        return 20  # If Correct

    def reward(self, belief):
        return self._reward(belief)
    ...
    def reset(self):
        ...
        import os
        eps = os.listdir("static/eps/configs")
        eps_id = sample(eps, 1)[0]
        self.eps_id = eps_id
        self.blicketProblem = config_control(1, self.config_size, eps_id)[0]
        # if random
        ...
        self.lastBelief = np.zeros(
            (self.config_size, ), dtype=np.float32) + 0.5

        observation = {
            "available_objects":
            self.available_objects,
            "context":
            self._list2array(self.history[self.stepcnt].objects),
            "light_state":
            np.array(
